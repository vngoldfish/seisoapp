...
export const HotelDetailsView: React.FC<HotelDetailsViewProps> = ({
  ...
  const { isLocked, getCurrencySymbol, darkMode } = useApp();
  ...
  const branchStats = useMemo(() => {
    ...
    const logsWithDuration = rangeLogs.filter(l => l.durationMinutes > 0);
    const totalDuration = logsWithDuration.reduce((acc, log) => acc + log.durationMinutes, 0);
    const finishedCount = rangeLogs.length;
    const avgCleaningTime = logsWithDuration.length > 0 ? Math.round(totalDuration / logsWithDuration.length) : 0;

    // Cleaner productivity leaderboard
    const cleanerMap: Record<string, { name: string; count: number; totalDuration: number; durationCount: number }> = {};
    rangeLogs.forEach(log => {
      const key = log.cleanerName || log.cleanerId || 'Unknown';
      if (!cleanerMap[key]) {
        cleanerMap[key] = { name: key, count: 0, totalDuration: 0, durationCount: 0 };
      }
      cleanerMap[key].count += 1;
      if (log.durationMinutes > 0) {
        cleanerMap[key].totalDuration += log.durationMinutes;
        cleanerMap[key].durationCount += 1;
      }
    });

    const leaderboard = Object.values(cleanerMap).map(c => {
      const avgTime = c.durationCount > 0 ? Math.round(c.totalDuration / c.durationCount) : 0;
      let speedCategory: 'fast' | 'normal' | 'slow' = 'normal';
      if (avgTime > 0 && avgTime < 30) speedCategory = 'fast';
                    ...
                    <div style={{ width: '100%', display: 'flex', flexDirection: 'column' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', marginTop: '0.75rem', fontSize: '0.7rem', fontWeight: 700, flexWrap: 'wrap' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                          ...
                          <span>{language === 'vi' ? `Phòng DND (${totals.dnd})` : language === 'ja' ? `起こさないde (${totals.dnd})` : `DND Rooms (${totals.dnd})`}</span>
                        </div>
                      </div>
                    </div>
                  );
